...
class SysvSharedMemoryLinkedList extends SysvSharedMemory implements Iterator, ArrayAccess, Countable, IExtendedIterator {
        const VAR_INIT            = 0;
        ...
        public function toArray(): array {
            ...
            SysvSharedMemoryLinkedList::rewind();

            while(SysvSharedMemoryLinkedList::valid()) {
                $nodes[] = SysvSharedMemoryLinkedList::current();
                SysvSharedMemoryLinkedList::next();
            }
        ...
        public function dequeue(): mixed {
            if($this->isEmpty())
                throw new StackUnderflowException("This linked list is empty.");

            $head = $this->pull(static::VAR_HEAD_POINTER);
            $value = $this->pull($head)[0];

            $this->offsetUnset($head - $this->pull(static::VAR_ROWS_START));
