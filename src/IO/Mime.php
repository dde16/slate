...
namespace Slate\IO {
    class Mime {
        ...
        public static function toExtensions(string $mime): array|null {
            return @static::$mimes["extensions"][$mime];
        }

        public static function toExtension(string $mime): string|null{
            if($extensions = static::toExtensions($mime)) {
                return $extensions[0];
            }
        ...
        public static function mimeByPath($path): string|null {
            if($mimes = self::mimesByPath($path)) {
                return $mimes[0];
            }
        ...
        public static function mimesByExtension($extension): array|null {
            if(array_key_exists($extension, self::$mimes["mimes"]))
                return self::$mimes["mimes"][$extension];

            return null;
        }

        public static function mimeByExtension($extension): string|null {
            if($mimes = self::mimesByExtension($extension)) {
                if(count($mimes) > 0) {
                    return $mimes[0];
                }
            }
        ...
        public static function mimesByPath($path): array|null {
            $pattern = "/^(\/?([\w^ ]+)+)*\/?([\w.]+\.([\w.]+))$/";
            $matches = array();
            ...
            if(preg_match($pattern, $path, $matches)) {
                ...
                return self::mimesByExtension($extension);
            }
